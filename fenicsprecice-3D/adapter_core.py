...
def determine_function_type(input_obj, dim = 2):
    """
    ...
    if isinstance(input_obj, FunctionSpace):  # scalar-valued functions have rank 0 is FEniCS
        ...
        elif input_obj.num_sub_spaces() == dim:
...
def get_forces_as_point_sources(fixed_boundary, function_space, data):
    ...
    z_forces : list or None
        If 3D, dictionary carrying Z component of forces with reference to each point on the coupling interface.
    """
    fenics_vertices = np.array(list(data.keys()))
    nodal_data = np.array(list(data.values()))

    # Check for shape of coupling_mesh_vertices
    n_vertices, dim = fenics_vertices.shape
